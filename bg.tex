...
A substitution is function $\sigma: \vars \to \ts_\Sigma$ mapping variables to terms.
...
Along with the equational theory, the protocol and attacker behavior are specified by a set of multiset rewriting rules.
...
Two terms $s$ and $t$ and \emph{equal modulo $E$} if $t =_E s$.
...
In particular, sort $\fresh$ models fresh, unguessable values such as cryptographic random numbers.
...
When not explicitly mentioned and not clear from context, a variable without prefix or sort annotation is of sort $\msg$.
...
The corresponding \emph{rewrite relation} $\to_R$ is defined as $s \to_R t$ if there is a position $p$ in $s$, rewrite rule $l \to r \in \R$ and substitution $\sigma$ such that $s|_p = l\sigma$ and $t = s[r\sigma]_p$.
...
The \emph{rewrite relation} $\to_{\R,AX}$ is defined as $s \to_{\R,AX} t$ if there is a position $p$, a rewrite rule $l \to r \in \R$, and subtitution $\sigma$ such that $s|_p =_{AX} l\sigma$ and $t = s[r\sigma]_p$.
...
% TODO: Simon here writes (\Sigma, \R^\simeq \cup AX) is an equational presentation of =_E, but I don't understand this really so opting for this definition. Need to check whether that's fine.
If $\R^\simeq \cup AX = E$ and $\R$ is $AX$-convergent and $AX$-coherent, then $t =_E s$ if and only if $t\normf^\R =_{AX} s\normf^\R$.
...
Facts typically model the local state of participants or the network, e.g., who posses which keys, or what messages have been sent out on the network.
...
$$[ \fact{Fr}(\ofsort{x}{\fresh}) ] \lrule{\Kup(\ofsort{x}{\fresh})} [ \Kup(\ofsort{x}{\fresh}) ].$$
...
$ri$ can be applied if its premises are contained in in the global state, which is then updated accordingly.
...
\begin{enumerate}
  ...
  \end{enumerate}
\end{enumerate}
Trace formulas can be combined with the logical connectives $\neg$, $\land$ and $\fresh$ and $\msg$ variables can be existentially quantified.
...
As all functions in $\Sigma$ (except for $+$) are of sort $\msg$, this means that variables of sort $\msg$ and $\sortnat$ are interpreted as ground terms, and $\fresh$, $\tmp$, and $\pub$ as constants.
...
We extend the notion of complete variants to multiset rewriting rules and multiset rewriting systems straightforward.
