...
If an equational theory $E$'s equations can be oriented to a convergent rewriting system $\R$, then $t =_E s$ if and only if $t{\downarrow}_\R = s{\downarrow}_\R$.
...
These symbols respectively model generating a fresh, unguessable value, receiving and sending a message over an insecure network, and adversary knowledge.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item all $\fresh$ and $\msg$ variables used in a rule's conclusion are introduced in a rule's premise.
\end{enumerate}
...
For this work, the following rule in $ND$ is particularly relevant:
\begin{gather*}
