...
\begin{proof}
  Let $NDG := \{ dg\normf^\R \mid dg \in \dgraphs_{AX}(\variants{R}^\R) \}$ and $NDG_{\neg\bot} := \{ dg\normf^\R \mid dg \in \dgraphs_{AX}(\variants{R}^\R \setminus \variants{R}^{\R,\bot})\}$.
  ...
  By assumption, we know that all rule instance $ri \in I$ are $\normf^\R$-normal (as $dg$ is $\normf^\R$-normal).
...
We can use this theorem to specialize Theorem~\ref{thm:reasoning-mod-ax} into the following one.
