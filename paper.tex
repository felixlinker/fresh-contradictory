...
While the optimization may seem conceptually simple, I would have neither been able to identify it nor to been able to prove its soundness prior to my Doctorate.
This work showed to me how much David had taught me during my Doctorate, which is why I would like to dedicate this work to David.
...
Variants extend to more complex constructs, like protocol rules, and Tamarin computes protocol rule variants during a pre-computation phase.
...
Our optimizations have been integrated into Tamarin's code base.\footnote{%
...
Our optimization applies to theories that use operators which can cancel out and will lead to multiple, e.g., rule variants when used.
For example, Tamarin provides built-in support for an equational theory modelling Diffie-Hellman groups.
That theory supports the operators $(\cdot)/2$ (multiplication), $\circ^\circ/2$ (exponentiation), $\circ^{-1}/1$ (inverse), and $\epsilon/0$ (neutral element) as well as, in particular, the equations:
\begin{gather*}
  x \cdot x^{-1} = \epsilon \\
  x^\epsilon = x
\end{gather*}
...
In the above rule, a protocol participant receives some message $m$ over the network, raises it by a fresh, unguessable value $x$, and sends the result over the network.
During pre-computation, Tamarin will consider the case that raising $m$ by $x$ might cancel out, which it represents with the rule variant:
$$[ \Fr(\ofsort{x}{\fresh}), \In(z^{\ofsort{x}{\fresh}^{-1}}) ] \rightarrow [ \Out(z) ].$$

Note, though, that constructing $m = z^{\ofsort{x}{fresh}^{-1}}$ requires knowledge of $x$ to $z$ by its inverse.
$x$, though, is freshly introduced in the above rule.
Thus, $m$ requires that $x$ is used before it is introduced, which is impossible.

In the following, we introduce \emph{fresh-contradictory rule variants}, which formalize the above intuition.
Fresh-contradictory rule variants use a fresh value before it was introduced.
We show that one can remove such variants from a protocol model without changing as the model's associated set of traces (normal form dependency graphs).
...
During a pre-computation phase, Tamarin computes rule variants.
Our optimization identifies redundant rule variants, which we call fresh-contradictory can which be omitted from pre-computation and thus also from proof search.

\paragraph{Future Work}
We identified the presented optimization during proof search where we observed that the adversary would use fresh values before they are introduced.
Tamarin already recognizes the use of fresh values before they are introduced as a contradiction, however, only when they occur not below a reducible function symbol such as exponentiation.
